...
\small
  ...
  \State $\InitialCandidates_s \gets$ filter and calculate the initial candidates \label{alg:MAP:mine} %\Comment{compute the initial candidates}
    \State $\Tract \gets$ convert data into transactional format from $\InitialCandidates_s$
...
\small
  ...
  \State $\CandidatesMap \gets (\emptyset, \emptyset)$
  \State $\TIDList \gets (\emptyset, \emptyset)$
  \State $\InitialCandidates \gets$ filter and calculate the initial candidates \label{alg:MAS:mine} %\Comment{compute the initial candidates}
  \State $\Tract \gets$ convert data into transactional format from $\InitialCandidates$
